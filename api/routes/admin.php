...
if ($path === '/admin/login' && $method === 'POST') {
    ...
    $u = DB::queryOne('SELECT id,company_id,role,email,password_hash,first_name,last_name,notify_emails FROM users WHERE email=? LIMIT 1', [$em]);
    ...
    jsonOut(['token' => $token, 'user' => ['id' => (int)$u['id'], 'companyId' => ($u['role']==='superadmin' && empty($u['company_id'])) ? null : (int)$u['company_id'], 'role' => $u['role'], 'email' => $u['email'], 'firstName' => $u['first_name'], 'lastName' => $u['last_name'], 'notifyEmails' => (bool)$u['notify_emails']]]);
}
...
if ($path === '/admin/me') {
    ...
    if ($method === 'GET') {
        $a = getAdmin($auth);
        $u = DB::queryOne('SELECT id,company_id,role,email,first_name,last_name,notify_emails FROM users WHERE id=? LIMIT 1', [(int)$a['id']]);
        jsonOut(['user' => ['id' => (int)$u['id'], 'companyId' => ($u['role']==='superadmin' && empty($u['company_id'])) ? null : (int)$u['company_id'], 'role' => $u['role'], 'email' => $u['email'], 'firstName' => $u['first_name'], 'lastName' => $u['last_name'], 'notifyEmails' => (bool)$u['notify_emails']]]);
    }

    if ($method === 'PATCH') {
        $b = getJson();
        if (!array_key_exists('notifyEmails', $b ?? [])) errOut('Chybí notifyEmails.');
        $ne = $b['notifyEmails'] ? 1 : 0;
        DB::exec('UPDATE users SET notify_emails=? WHERE id=?', [$ne, (int)$a['id']]);
        jsonOut(['ok' => true, 'notifyEmails' => (bool)$ne]);
    }
...
if ($path === '/admin/companies') {
    ...
    if ($method === 'GET') {
        ...
        $users = DB::query("SELECT id,company_id,role,email,first_name,last_name,phone,notify_emails,current_credit,created_at FROM users WHERE company_id IN ($ph) ORDER BY company_id,id", $ids);

        $ubc = []; foreach ($users as $u) { $ubc[$u['company_id']][] = ['id' => (int)$u['id'], 'companyId' => (int)$u['company_id'], 'role' => $u['role'], 'email' => $u['email'], 'firstName' => $u['first_name'], 'lastName' => $u['last_name'], 'phone' => $u['phone'], 'notifyEmails' => (bool)$u['notify_emails'], 'currentCredit' => (float)$u['current_credit'], 'createdAt' => $u['created_at']]; }
...
if ($path === '/admin/users') {
    ...
    if ($method === 'GET') {
        ...
        $rows = DB::query('SELECT id,company_id,role,email,first_name,last_name,phone,notify_emails,current_credit,created_at FROM users ' . ($cid ? 'WHERE company_id=?' : '') . ' ORDER BY company_id,id', $cid ? [$cid] : []);
        jsonOut(array_map(fn($u) => ['id' => (int)$u['id'], 'companyId' => (int)$u['company_id'], 'role' => $u['role'], 'email' => $u['email'], 'firstName' => $u['first_name'], 'lastName' => $u['last_name'], 'phone' => $u['phone'], 'notifyEmails' => (bool)$u['notify_emails'], 'currentCredit' => (float)$u['current_credit'], 'createdAt' => $u['created_at']], $rows));
    }
    ...
    if ($method === 'POST') {
        ...
        $pwh = hashPw($pw);
        $ne = array_key_exists('notifyEmails', $b ?? []) ? ($b['notifyEmails'] ? 1 : 0) : 1;
        $uid = DB::insert("INSERT INTO users (company_id,role,email,password_hash,first_name,last_name,phone,notify_emails,current_credit) VALUES(?,?,?,?,?,?,?,?,0.00)", [$cid, $role, $em, $pwh, $fn, $ln, $ph ?: null, $ne]);
        $cr = DB::queryOne('SELECT id,company_id,role,email,first_name,last_name,phone,notify_emails,current_credit,created_at FROM users WHERE id=? LIMIT 1', [$uid]);
        jsonOut(['id' => (int)$cr['id'], 'companyId' => (int)$cr['company_id'], 'role' => $cr['role'], 'email' => $cr['email'], 'firstName' => $cr['first_name'], 'lastName' => $cr['last_name'], 'phone' => $cr['phone'], 'notifyEmails' => (bool)$cr['notify_emails'], 'currentCredit' => (float)$cr['current_credit'], 'createdAt' => $cr['created_at']], 201);
    }
...
if (preg_match('#^/admin/users/(\d+)$#', $path, $m)) {
    ...
    if ($method === 'PATCH') {
        ...
        if ($ph) { $f[] = 'phone=?'; $v[] = $ph; }
        if (array_key_exists('notifyEmails', $b ?? [])) { $f[] = 'notify_emails=?'; $v[] = $b['notifyEmails'] ? 1 : 0; }
        ...
        $up = DB::queryOne('SELECT id,company_id,role,email,first_name,last_name,phone,notify_emails,current_credit,created_at FROM users WHERE id=? LIMIT 1', [$uid]);
        jsonOut(['id' => (int)$up['id'], 'companyId' => (int)$up['company_id'], 'role' => $up['role'], 'email' => $up['email'], 'firstName' => $up['first_name'], 'lastName' => $up['last_name'], 'phone' => $up['phone'], 'notifyEmails' => (bool)$up['notify_emails'], 'currentCredit' => (float)$up['current_credit'], 'createdAt' => $up['created_at']]);
    }
