...
if (preg_match('#^/admin/reservations/(\d+)/approve$#', $path, $m)) {
    ...
    $ex = DB::queryOne('SELECT id,company_id,status,user_id FROM reservations WHERE id=? LIMIT 1', [$rid]);
    ...
    try {
        ...
        $user = DB::queryOne('SELECT email, first_name, last_name FROM users WHERE id=? LIMIT 1', [(int)$ex['user_id']]);
        $slots = DB::query('SELECT date, time_start FROM reservation_slots WHERE reservation_id=? ORDER BY date, time_start', [$rid]);
        if ($company && $user && $slots) {
            $ref = ['date' => $slots[0]['date'], 'slotStarts' => array_column($slots, 'time_start')];
            $mailer->sendConfirmation($company, ['firstName' => $user['first_name'], 'lastName' => $user['last_name'], 'email' => $user['email']], $ref, $rid);
        }
...
if (preg_match('#^/admin/reservations/(\d+)/cancel$#', $path, $m)) {
    ...
    if (!$rid) errOut('Neplatné ID rezervace.');
    $ex = DB::queryOne('SELECT id,company_id,status,note,user_id FROM reservations WHERE id=? LIMIT 1', [$rid]);
    if (!$ex) errOut('Rezervace neexistuje.', 404);
    if (!isSA($a) && (int)$ex['company_id'] !== (int)$a['company_id']) errOut('K této rezervaci nemáte přístup.', 403);
    ...
    try {
        ...
        if ($company && $user && $slots) {
            $ref = ['date' => $slots[0]['date'], 'slotStarts' => array_column($slots, 'time_start')];
            $mailer->sendCancellation($company, ['firstName' => $user['first_name'], 'lastName' => $user['last_name'], 'email' => $user['email']], $ref, $rid, $reason);
        }
    } catch (\Throwable $e) { /* email selhal — nevadí */ }
...
if ($path === '/admin/email-logs') {
    ...
    $rows = DB::query(
        'SELECT id, reservation_id, recipient_email, recipient_name, type, subject, sent_at
         FROM email_logs
         WHERE company_id=?
...
if ($path === '/admin/email-templates') {
    ...
    if ($method === 'PUT') {
        $b = getJson();
        $type = trim($b['type'] ?? '');
        if (!in_array($type, ['customer_summary', 'confirmation', 'cancellation'], true)) {
            errOut('Typ šablony musí být customer_summary, confirmation nebo cancellation.');
        }
